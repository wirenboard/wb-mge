...
| 343         | 0x0157        | 1         | u16    | Таймаут значения кэша, секунды                                              |
...
- **Статистика шины** (337–343) берётся из мультимастер-кэша; при неактивном кэше
  соответствующие поля читаются как 0. Регистр 336 (0x0150) намеренно пропущен: это последний
  регистр стандартного поля версии загрузчика Wiren Board (330–336), поэтому он не определён.
