...
GUARDS — the test PASSES ON DEFECTIVE FIRMWARE. With them such a run is reported as SKIPPED
with the reason "THIS RUN PROVES NOTHING", tagged DETECTOR-DISARMED so it can be counted
across builds; that is the honest verdict, since the guards make the disarmed run visible but
say nothing about the firmware either way. NOT red: a message whose own words are "this is NOT
a firmware failure" must not paint the build UNSTABLE, or a busy node reads as a product
regression and the gate becomes one people learn to ignore. Nothing that accuses the firmware
was weakened for it — the step 11 probe's ReadTimeout, step 12, the step 6/10 preconditions and
guard B's split precondition all still fail — but note that the skip ENDS THE BODY, so steps
15-16 and the WEDGED assert after the `try/finally` are not reached on that path; `finally`
still runs and its printed WEDGED line is then the only blast-radius signal. See the note at
guard B in step 13 and _skip_detector_disarmed. The hang therefore only reproduces while the
...
run whose receiver left through EAGAIN is skipped as inconclusive whatever the firmware. That
...
# Deliberately wider than WBMGEAPI's 30 s scalar default (api_client.py:94/:98), and a TUPLE
...
# guard reacts to it. Guard A measures only intervals in which the client had NOTHING
# outstanding (see step 13 and _longest_send_gap), so a throttled tick cannot produce a guard A
# skip at all. That scoping is not a nicety: build #21 failed a perfectly valid run because
...
# A as NOT APPLICABLE instead of drawing a verdict from numbers it knows to be wrong — and that
# cuts both ways now: skipping on them would tag the run DETECTOR-DISARMED and inflate that
# count with runs nothing was wrong with. Guard B is
# unaffected: it reads two single timestamps, not these logs. Unreachable at today's constants
# (600 s x 50/s = 30 000 < 60 000), but 600 is a marker constant that can rise, and the verdict
# it would produce would look exactly like a real one.
_SEND_LOG_MAX = 60000
...
# Stable, greppable marker on every run this file abandons because the ENVIRONMENT invalidated
# the measurement. Counting these across builds is the point — see _skip_detector_disarmed.
_DISARMED_MARKER = "DETECTOR-DISARMED"


def _skip_detector_disarmed(reason: str) -> typing.NoReturn:
    """End the item as SKIPPED: the environment disarmed the detector, the firmware is not accused.

    WHY SKIP AND NOT FAIL. The step 13 guards decide whether THIS RUN was capable of detecting
    the regression at all, and each of their messages says in its own words that it is not a
    firmware verdict ("this is NOT a firmware failure", "the run did not exercise the
    deinit-with-active-client path"). A red item paints the build UNSTABLE
    (Jenkinsfile: catchError buildResult 'UNSTABLE'), so a busy node would read as a product
    regression — and once the e2e gate is made blocking, that is exactly the gate people learn
    to ignore. Nothing that accuses the firmware was weakened to get there: the step 11 probe's
    ReadTimeout, step 12's `assert probe_ok`, the step 6/10 preconditions and the split guard-B
    precondition all still fail.

    WHAT THIS PATH DOES NOT REACH, which is not a weakening but is worth knowing. This raises
    inside the item's `try`, so steps 15-16 never run and the liveness check at step 16 is not
    evaluated; `finally` still runs (client stop, settings restore, blast-radius GET), but the
    `assert wedged_reason is None` that follows sits AFTER the `try/finally` and is skipped with
    the rest of the body. So on a disarmed run against a WEDGED device the only blast-radius
    signal is the `WEDGED:` line printed from `finally` — the same signal a ReadTimeout failure
    leaves, and for the same structural reason.

    WHY THE MARKER, AND WHY IT IS ALSO PRINTED. A skip nobody counts is how a test quietly
    stops running forever; the marker is what makes "how often does this node disarm the
    detector" a grep rather than a guess. Under this suite's addopts (api_tests/pytest.ini:
    -v -s --tb=short, no -r flag) pytest truncates the reason in its own result line to
    `SKIPPED (DETECTOR-DISARME...)`, cutting the marker mid-word, so neither artefact would
    carry it without help. Where the full text DOES land:
      - build/qemu_test_report.xml, as <skipped message="...">. qemu.mk:278-280 passes
        --junitxml, and the Jenkinsfile stage 'E2E tests (QEMU)' publishes that file from its
        `post { always { ... } }` block — the `junit` step for the Tests tab, `archiveArtifacts`
        for the raw XML — so it survives whatever the build result was. Cited by stage and step
        name rather than by line number: line numbers there have gone stale twice already.
    ...
    `reason` is passed through UNCHANGED apart from the marker prefix. It is the only record of
    why the run was invalid, and a skipped run has no traceback to fall back on.
    """
...
def _longest_send_gap(send_times, response_times, t0: float, t1: float):
    ...
    silence is a millisecond of the timeout it is counting down.  Guard A decides on this, and
    skips the run when it exceeds the firmware's SO_RCVTIMEO.
    ...
    "inconclusive" (a skipped run on healthy firmware), never toward a false pass.
...
def test_cache_server_deinit_with_active_polling(api):
    ...
    established positively.  Connectedness: step 6 requires the polling client's
    ...
    HOW A FAILED FEED-RATE CHECK IS REPORTED: the item ends as SKIPPED, not
    failed, with a reason carrying the DETECTOR-DISARMED marker (see
    _skip_detector_disarmed).  Guard B, guard A and the "the drainer never saw
    the teardown" half of guard B's precondition all conclude that THIS RUN
    measured nothing — a statement about this node, not about the firmware — and
    a red item would report that as a product regression.  What still FAILS is
    every conclusion about the firmware: the probe's ReadTimeout (the hang
    itself), step 12, steps 6 and 10, the liveness check, the WEDGED assert, and
    the other half of guard B's precondition — a drainer that saw the connection
    end WITHOUT a peer close, which means the response stream went out of sync.
    Note that the skip ends the body, so steps 15-16 and the WEDGED assert after
    the `try/finally` are not reached on that path.
    ...
    try:
        ...
        # _CLOSE_OBSERVE_MAX_S waiting for a teardown that cannot come and then reports on a
        # false premise, while the true diagnosis — the one below — never prints.
        #
        # SINCE THE STEP 13 GUARDS SKIP RATHER THAN FAIL, THIS ORDERING MATTERS MORE, NOT LESS.
        # It used to cost only the quality of the diagnosis: red either way, wrong explanation.
        # Now guard B's validity half would end that run as a SKIPPED item — a rejected probe
        # leaves the deinit possibly still hung and the connection still up, so the drainer sees
        # no ending and the disarmed branch is exactly the one that fires. A real, red,
        # firmware-accusing failure would be laundered into a green build under a
        # DETECTOR-DISARMED tag. Step 12 stays ABOVE step 13; do not reorder them.
        #
        ...
        # that fails it is INCONCLUSIVE, which must read as a SKIP and not as a pass. Not as a
        # failure either: the verdict is about this node, not about the firmware. See the note
        # at guard B for why, and for the DETECTOR-DISARMED marker that makes such runs
        # countable rather than merely invisible.
        #
        ...
        # stall is measured in full and skips the run as inconclusive — correctly, because a
        ...
        #     can cross the limit, i.e. a skipped "proves nothing" on good firmware. This is what
        ...
        #     turn a healthy run into a skip. It is the reason the limit is not TIGHTENED toward
        #     3-6 ms.
        ...
        #
        # THE TWO SIDES ARE NO LONGER "RED vs GREEN" BUT "SKIP vs PASS", AND THAT WIDENS THE
        # GAP BETWEEN THEIR COSTS RATHER THAN NARROWING IT. Erring to the safe side used to
        # cost a red build on healthy firmware — expensive enough that tightening the limit was
        # a real temptation. It now costs a skipped item on a green build, which is close to
        # free. Erring the other way still costs what it always did: a PASS on firmware that
        # took the EAGAIN exit, i.e. the regression shipping undetected under a green tick.
        # So the headroom above the healthy distribution is worth at least as much as before,
        # and probably more: the only argument for shaving it was a cost this change removed.
        # Do not tighten _CLOSE_GAP_MAX_S toward the healthy 3-6 ms on the grounds that a
        # disarmed run is now cheap — cheap is the reason to keep the margin, not to spend it.
        #
        ...
        # so it is skipped as inconclusive — even if the firmware is in fact healthy.
        ...
        # THIS "THIS RUN PROVES NOTHING" DELIBERATELY STAYS AN ASSERT, unlike the three in step
        # 13 below. Recorded here because the inconsistency is otherwise the kind a later reader
        # "fixes" in the wrong direction: the message is worded like a validity guard, but what
        # can actually reach it is not.
        #
        # Reachability. Failing needs connection_ended_at <= t0. Step 10 has already run, so
        # connection_ended_at being set at all implies closed_by_peer — a host-side death
        # (_end_connection without _note_peer_close) fails there instead. And it must have been
        # stamped at or before t0, which step 6 pins to essentially the same instant: the settle
        # loop had just watched the response counter move, and t0 is taken a few statements
        # later. Note the sender can be the stamping thread — it meets the RST on its next 20 ms
        # tick via _note_peer_close while the drainer sits in recv() for up to a second — so
        # is_alive() being True at step 6 does not exclude it.
        #
        # What is left is the firmware closing a HEALTHY, actively-served client before it was
        # asked to do anything. That is a product fact, not a busy node, and it must stay red.
        ...
        if stats.closed_by_peer:
            ...
            ending = "still up"
        # Two host-side figures, and only the second is decided on. `gap_all` is the longest
        ...
        #
        # ITS PRECONDITION IS SPLIT IN TWO, because "the drainer did not report a peer close"
        # is not one fact but two, and they get opposite verdicts. A single condition covering
        # both was correct while both FAILED; the moment the validity half became a skip it
        # would have reported a firmware signature as "not our problem" — a silent downgrade,
        # which is the specific way this kind of change goes wrong.
        #
        # (i) THE DRAINER ENDED, BUT NOT BY A PEER CLOSE — a PRODUCT fact, and it still fails.
        # That state is written by _drainer_end(..., peer_closed=False) in the
        # `except (OSError, ValueError, struct.error)` branch of _drain_loop, and the reachable
        # half of that branch is the ValueError from the MBAP plausibility check: the guest sent
        # bytes this client cannot parse as a Modbus frame, i.e. the cache server's response
        # stream went OUT OF SYNC. Nothing about a loaded node produces that.
        #
        # WHY THE OTHER MEMBERS OF THAT except CANNOT GET HERE — an argument about the DRAINER
        # only. (An earlier revision argued it from _MAX_OUTSTANDING and the sender's
        # socket.timeout branch, which is doubly wrong: those are sender-side, and the sender
        # cannot write drainer_ended_at at all — it calls _note_peer_close/_end_connection,
        # never _drainer_end. Right conclusion, unrelated evidence.) The real reasons are
        # structural, all inside _drain_loop and _recv_frame_bytes:
        #   - Every peer-initiated ending is claimed by the EARLIER handler. ConnectionError is
        #     an OSError subclass and its `except` is listed FIRST (:931 vs :945), and the
        #     orderly close — recv() returning b"" — is converted into one by
        #     _recv_frame_bytes (:538-539). So a reset, an RST and a FIN all land in the
        #     peer_closed=True branch, never here.
        #   - socket.timeout never escapes _recv_frame_bytes (:531-537): it is swallowed with a
        #     `continue`, or returns None once the sender has declared the connection dead. That
        #     matters precisely because socket.timeout IS an OSError — unswallowed, a merely
        #     slow guest would arrive here and be blamed for a framing defect.
        #   - Our own teardown is filtered by the stop_event check at :946 (stop() sets the
        #     event at :767, before the shutdown() at :770 that wakes this thread).
        #   - struct.error cannot fire: the header is exactly 8 bytes by construction (:952-954).
        # What remains alongside the ValueError is an exotic host-side OSError (EBADF and the
        # like), which would be a defect of this harness rather than a slow node — so no
        # plausible path here is "the environment was busy", which is what the skip column is
        # for.
        #
        # STEP 10 DOES NOT ALREADY COVER THIS, and an earlier revision of this comment wrongly
        # said it did. Step 10 is evaluated the instant the trigger POST returns, while this
        # window stays open for the whole probe — seconds on the CI node — so a stream that
        # desynchronises inside it is visible HERE and nowhere else in the item.
        #
        # (ii) THE DRAINER NEVER OBSERVED AN ENDING AT ALL — a VALIDITY fact, and it skips. The
        # measuring apparatus lost the event it measures from: the teardown went unseen, or the
        # sender won the race to observe it. That is a property of THIS RUN's scheduling on THIS
        # node, and the message says as much ("the run did not exercise the
        # deinit-with-active-client path"). A verdict that denies a firmware defect must not
        # paint the build UNSTABLE: a loaded node then reads as a product regression, and once
        # the e2e gate is made blocking that is precisely the gate people learn to ignore. The
        # same reasoning covers the two guards below and nothing else in this file — every
        # guard whose conclusion is "the firmware did the wrong thing" still FAILS, and must.
        #
        # The marker and the reasoning live in _skip_detector_disarmed, which also prints the
        # reason so the CI console log — not just the JUnit XML — can be grepped for it. The
        # diagnostic text below is what it carries, UNCHANGED: it is the only record of why the
        # run was invalid, and a skipped run has no traceback to fall back on.
        if stats.drainer_ended_at is not None:
            assert stats.drainer_closed_by_peer, (
                f"the drainer stopped reading this connection for a reason that was NOT the "
                f"firmware closing it: {stats.error}. The reachable cause is the MBAP "
                f"plausibility check in _drain_loop — the guest sent bytes this client cannot "
                f"parse as a Modbus frame, so the cache server's response stream went out of "
                f"sync. That is a FIRMWARE fact, not a slow node, which is why it fails here "
                f"instead of being reported as an inconclusive run. Step 10 does not cover it: "
                f"that check runs the moment the trigger POST returns, while this window stays "
                f"open for the whole probe (seconds on the CI node). "
                f"drainer_ended_at={stats.drainer_ended_at!r}, last_response_at="
                f"{stats.last_response_at!r}, responses={stats.responses}, cross-thread view: "
                f"closed_by_peer={stats.closed_by_peer}, "
                f"connection_ended_at={stats.connection_ended_at!r}"
            )
        if not (stats.drainer_closed_by_peer and close_gap is not None):
            _skip_detector_disarmed(
                ...
                f"and the drainer did not, and the two timestamps must not be subtracted from "
                f"each other. Either way: whatever made the count reach 0, this client did not "
                f"watch the receiver task let go of THIS connection, so the run did not "
                f"exercise the deinit-with-active-client path"
            )
        # Guard B's own threshold, and the clearest case of the rule above: this message ENDS
        # with "this is NOT a firmware failure". A run that says that in its own words cannot
        # be allowed to turn the build yellow. Same marker, same verbatim text; skip, not fail.
        if close_gap >= _CLOSE_GAP_MAX_S:
            _skip_detector_disarmed(
                ...
                f"does NOT touch — not of the post-data check at :426-429 that this test exists "
                f"to guard. A defective receiver leaves exactly this way and its deinit completes "
        ...
        # GUARD (A), host-side hygiene only — neither necessary nor sufficient, and scoped to
        # the DRAINED part of the window (it skips the run, it does not fail it). It is not a
        # necessary condition on every
        ...
        #
        # NOT EVALUATED — this GUARD only, the item keeps running — and not softened, when the
        # send log overflowed. Distinct from the skip below, which ends the item: past
        # _SEND_LOG_MAX the logs stop recording while the connection keeps working, and
        # _longest_send_gap's trailing term then measures "time since the last RECORD", not
        # "time since the last SEND", while the reconstructed `outstanding` drifts from the
        # truth in the other direction. Asserting on either would be asserting on a number
        # known to be wrong — and so would SKIPPING on it, which would tag the run
        # DETECTOR-DISARMED on evidence that is itself unsound and inflate that count with
        # runs nothing was actually wrong with. Guard B has already run and carries the
        # verdict. Unreachable at today's
        ...
        if stats.send_log_truncated:
            ...
            # SKIP, NOT FAIL, for the reason spelled out at guard B above. This message is if
            # anything more explicit: "This is NOT a firmware failure and must not be read as
            # one: this PROCESS missed its own send schedule". The accused party is the host,
            # so the verdict belongs in the skip column, not in the one that turns a build
            # yellow and gets read as a product regression.
            #
            # The message's closing clause — "failing loudly here is the correct outcome" —
            # predates this change and is preserved VERBATIM, because the requirement is that
            # the diagnostic survives unedited: it is the only record of why the run was
            # invalid. Read "loudly" as it is now delivered: a skip reason carrying the whole
            # measurement, tagged DETECTOR-DISARMED so it can be grepped and counted across CI
            # builds. What is no longer claimed is that the FIRMWARE was at fault, which this
            # message never claimed in the first place.
            if longest_drained_gap >= _GUEST_RECV_TIMEOUT_S:
                _skip_detector_disarmed(
                    f"THIS RUN PROVES NOTHING about cache_modbus_server_deinit(): the polling "
                    f"client went {longest_drained_gap * 1000:.0f} ms without sending WHILE IT "
                    f"HAD NOTHING OUTSTANDING — every request it had sent was already answered, "
                    f"so the guest's receiver spent all of that back in recv() with an empty "
                    f"queue — over the {_GUEST_RECV_TIMEOUT_S * 1000:.0f} ms SO_RCVTIMEO set on "
                    f"every accepted socket (main/bridge/tcp_server.c:227-228). A gap that "
                    f"long lets even a "
                    ...
                    f"kind {gap_all} (not asserted on, and deliberately: while the guest still "
                    f"owes answers recv() returns immediately and no host-side pause can "
                    f"starve it), {stats.throttled_ticks} ticks skipped by the "
                    f"{_MAX_OUTSTANDING}-request "
                    ...
                    f"that cannot sustain a sub-{_GUEST_RECV_TIMEOUT_S * 1000:.0f} ms send "
                    f"cadence this test cannot detect its regression at all, and failing "
                    f"loudly here is the correct outcome"
                )
