...
#
# 600 s, matching the precedent at 33_test_auth_settings.py:314-319, repeated at :413-418,
# which argues the number out: a cold ESP32 boot under QEMU on a CPU-starved CI node "can take
# well over two minutes", so 600 s is ~5x the observed worst case. It replaces the 1800 s this
# file used before; 1800 s is the number the pre-reboot waits elsewhere in the suite use, but it
# does not fit any honest budget here (see the budget block below) and buys nothing above 600 s
# except a guarantee that pytest-timeout's SIGALRM, not this wait, is what ends a hung run.
...
# (api_client.py:22), rounded up to whole seconds. Connect is a loopback handshake — negligible.
_CALL_AUTH_S = 11      # WBMGEAPI.auth              (api_client.py:80,     timeout=10)
_CALL_INFO_S = 11      # WBMGEAPI.get_info          (api_client.py:87,     timeout=10)
_CALL_SETTINGS_S = 31  # WBMGEAPI.get/update_settings (api_client.py:94/98, timeout=30)
_CALL_CMD_S = 31       # WBMGEAPI.execute_command   (api_client.py:228,    timeout=30)
...
# (api_client.py:91-94), so an unreadable value says nothing about the port at all.
_UNREADABLE = object()
...
def test_web_port_change(api, is_qemu):
    ...
    try:
        ...
        # precisely what 33_test_auth_settings.py:169-174 spells out must never happen from a
