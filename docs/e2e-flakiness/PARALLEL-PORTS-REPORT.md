...
An investigation write-up: how the QEMU host ports were made dynamic so that several full e2e
runs can share one host, and what using those parallel runs as a flake stressor revealed. It
records the design rationale and the findings; it is not a changelog. The measurements were
taken with the test dependencies pinned as in `api_tests/requirements.txt` (pytest 9.1.1 /
pluggy 1.6.0).
...
### Two bugs the parallel validation caught (both introduced by Part 1 itself)
...
1. **guest/host port conflation** (fixed by the guest/host port split). The blanket rewrite
   made some firmware *settings* dynamic too. In the legacy scheme host == guest (50504 ==
   50504), so one constant served both; making everything dynamic told the cache firmware to
   listen on the dynamic host port while the forward still hit guest 50504 → connection
   refused. Diagnosed from the log: `Starting 3 parallel Modbus TCP clients on localhost:50504
   → FAILED` — the test connected to the guest value, which is not on the host under a non-zero
   slot. Broke the cache subsystem (11/20/31/37/41/42/43) identically on every slot
   (identical-across-slots = deterministic bug, not contention). Fix: `qemu_ports` exposes
   guest-port constants; cache tests set `cache_modbus_port` to the guest port and connect to
   the dynamic host port.

2. **slot-suffixed QEMU log** (fixed by restoring the fixed log name). The log had been
   suffixed (`qemu_test.slotN.log`) as a same-tree safety. But `33_test_auth_settings.py`
   stats the hardcoded `build/qemu_test.log` to get a baseline offset for detecting a SW
   reboot; with the suffix that path did not exist on slots 1/2/3, so `au05`/`au06` asserted
   out with `Cannot stat the QEMU serial log`. Reverted to the fixed name (the suffix was
   pointless — separate trees already prevent clobber).
...
`33_test_auth_settings.py::test_au05_full_buffer_preserved_after_sw_reboot`. The block below is
a HISTORICAL excerpt captured during this investigation — its line number is the one the file
had then and is not a live reference; locate the assertion by its message instead:
```
...
(again a HISTORICAL excerpt: the line number is the one the file had at capture time, not a
live reference — locate the assertion by its message):
```
...
2. **Parallelism is a real flake stressor** — it caught two defects a single run never would
   (both introduced by Part 1 itself: guest/host conflation; slot-suffixed log), each
   diagnosed to mechanism from the logs and fixed.
...
By content rather than by commit hash, since these changes reach `main` squashed and the
hashes they were developed under do not survive that:

- All host ports derived from one slot in the new `api_tests/qemu_ports.py`, with conftest,
  the test modules and the Makefile launchers rewired to it.
- Guest and host ports split: a firmware *setting* that names a port keeps the fixed guest
  value while the host side moves per slot. This is what unbroke the cache subsystem.
- QEMU log kept at the fixed `build/qemu_test.log`; a per-slot suffix had broken the
  reboot detection that stats that path.
- One run per working tree enforced (flock in `api_tests/tree_lock.py`, plus the targeted
  orphan-QEMU check for what flock cannot see).
- The slot wired into Jenkins from `EXECUTOR_NUMBER`; `qemu.mk`'s own launcher ports derived
  from `qemu_ports`, so `make qemu-web` + `pytest --ip` works end to end again; `--qemu`
  refused together with xdist; the UDP preflight port probed by bind instead of a TCP
  connect; the slot capped at the ephemeral floor; an empty `WB_MGE_PORT_SLOT` treated as
...
The harness that drove the runs reported above was ad-hoc and is NOT preserved in this
repository, so the numbers cannot be reproduced by re-running a script. What it did, for
whoever needs to recreate it: given N, a run tag and an optional pytest target, it started N
docker instances with `--network host` (the shared host netns is what reproduces the CI
collision — a default bridge isolates the netns and would not exercise the fix), each on its
own checkout of the tree and with `WB_MGE_PORT_SLOT` set to that instance's index, ran
`make qemu-test` (or pytest against the given target) inside each, and collected every
instance's `build/qemu_test.log` and `build/qemu_test_report.xml` under the run tag. For the
repeated-round sweeps it also reset each tree's `build/qemu_flash.bin` from a golden clean
image between rounds, which is what keeps the NVS coupling described above out of the
results.
