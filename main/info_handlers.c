...
esp_err_t info_build_rs485_json(cJSON **rs485_json)
{
    ...
    *rs485_json = cJSON_CreateObject();
    ...
    if (rs485_1 == NULL) {
        ESP_LOGE(TAG, "Failed to create RS485_1 JSON object");
    ...
    cJSON_AddItemToObject(*rs485_json, "rs485_1", rs485_1);

    // RS485_2 status
    cJSON *rs485_2 = cJSON_CreateObject();
    if (rs485_2 == NULL) {
        ESP_LOGE(TAG, "Failed to create RS485_2 JSON object");
        cJSON_Delete(*rs485_json);
        return ESP_FAIL;
    }
    ...
    cJSON_AddItemToObject(*rs485_json, "rs485_2", rs485_2);
...
esp_err_t info_get_handler(httpd_req_t *req)
{
    ...
    if (info_build_device_json(&device_json) == ESP_OK && device_json != NULL) {
        ...
        if (device_name) cJSON_AddItemToObject(response_json, "device_name", device_name);
        if (firmware) cJSON_AddItemToObject(response_json, "firmware", firmware);
        if (hardware) cJSON_AddItemToObject(response_json, "hardware", hardware);
        if (serial_num) cJSON_AddItemToObject(response_json, "serial_num", serial_num);
    ...
    if (info_build_network_json(&network_json) == ESP_OK && network_json != NULL) {
        ...
        if (ethernet) cJSON_AddItemToObject(response_json, "ethernet", ethernet);
        if (wifi) cJSON_AddItemToObject(response_json, "wifi", wifi);
    ...
    if (info_build_rs485_json(&rs485_json) == ESP_OK && rs485_json != NULL) {
        ...
        if (rs485_1) cJSON_AddItemToObject(response_json, "rs485_1", rs485_1);
        if (rs485_2) cJSON_AddItemToObject(response_json, "rs485_2", rs485_2);
