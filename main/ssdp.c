...
static void onPacket(int sock, in_addr_t remote_addr, uint16_t remote_port, char *buf, int len)
{
    ESP_LOGD(TAG, "received %d bytes from %s:%d", len, ip4addr_ntoa((const ip4_addr_t *)&remote_addr), remote_port);
    ESP_LOGD(TAG, "%s", buf);
    ...
    ESP_LOGD(TAG, "Success to get on packet semaphore");
                    ...
                    switch (header) {
                        ...
                        case ST:
                            ...
                            if (strcmp(buffer, "ssdp:all") == 0 || strcmp(buffer, "upnp:rootdevice") == 0) {
                                ...
                                ESP_LOGD(TAG, "the search type matches all and root\n");
                                state = KEY;
                            } else {
                                ...
                                if (strcasecmp(buffer, ssdp_task_config->device_type) == 0) {
                                    ...
                                    strlcat(ssdp_task_config->usn_suffix, ssdp_task_config->device_type,
                                            sizeof(ssdp_task_config->usn_suffix));
                                    // ESP_LOGD(TAG, "the search type matches our type %s\n");
                                    state = KEY;
                                } else {
                                    state = ABORT;
                                    ESP_LOGD(TAG, "REJECT. The search type %s does not match our type %s\n", buffer,
    ...
    if (pending) {
        ...
        ESP_LOGD(TAG, "SSDP: respond...\n");
    } else {
        ESP_LOGD(TAG, "SSDP: ignore...\n");
    }
...
void ssdp_send(int sock, ssdp_method_t method, in_addr_t remote_addr, uint16_t remote_port)
{
    ...
    ESP_LOGD(TAG, "Success to get send semaphore");
    int err = 0;
    if (method == NONE) {
        ESP_LOGD(TAG, "Sending Response to %s:%d", ip4addr_ntoa((const ip4_addr_t *)&remote_addr), remote_port);
        ...
        strlcpy(ssdp_task_config->usn_suffix, "::upnp:rootdevice", sizeof(ssdp_task_config->usn_suffix));
        ESP_LOGD(TAG, "Sending Notify to %s:%d", SSDP_MULTICAST_ADDR, SSDP_PORT);
    }
    size_t len_msg_template = (method == NONE) ? strlen(SSDP_RESPONSE_TEMPLATE) : strlen(SSDP_NOTIFY_TEMPLATE);
    size_t msg_buffer_size = strlen(SSDP_PACKET_TEMPLATE) + len_msg_template + 5 +
                             ...
                             (ssdp_task_config->model_number ? strlen(ssdp_task_config->model_number) : 1) + (SSDP_UUID_SIZE) +
                             (SSDP_USN_SUFFIX_SIZE) + 2  // "NT" or "ST"
    ...
    int result = snprintf(
        msg_buffer, msg_buffer_size, SSDP_PACKET_TEMPLATE, ((method == NONE) ? SSDP_RESPONSE_TEMPLATE : SSDP_NOTIFY_TEMPLATE),
        (unsigned int)ssdp_task_config->interval, ssdp_task_config->server_name ? ssdp_task_config->server_name : "",
        ssdp_task_config->model_name ? ssdp_task_config->model_name : "",
        ssdp_task_config->model_number ? ssdp_task_config->model_number : "",
        ssdp_task_config->uuid ? ssdp_task_config->uuid : "", ssdp_task_config->usn_suffix, (method == NONE) ? "ST" : "NT",
        ssdp_task_config->respond_type, ssdp_get_LocalIP(), ssdp_task_config->port,
        ssdp_task_config->schema_url ? ssdp_task_config->schema_url : "");
    ESP_LOGD(TAG, "sprintf result: %d", result);
    if (result < 0) {
        ESP_LOGE(TAG, "Error not enough memory for msg_buffer creation");
    }

    ESP_LOGD(TAG, "*************************TX*************************");
    ESP_LOGD(TAG, "%s", msg_buffer);
    ESP_LOGD(TAG, "****************************************************");
    ...
    if (method == NONE) {
        err = getaddrinfo(ip4addr_ntoa((const ip4_addr_t *)&remote_addr), NULL, &hints, &res);
    } else {
    ...
    ESP_LOGD(TAG, "Sending to IPV4 multicast address %s:%d...", ip4addr_ntoa((const ip4_addr_t *)&remote_addr), remote_port);
        ...
        while (err > 0) {
            ...
            if (s < 0) {
                ...
                if (FD_ISSET(multicast_socket, &rfds)) {
                    ...
                    // be discarded
                    int len = recvfrom(multicast_socket, ssdp_task_config->datagram_buffer, SSDP_DATAGRAM_SIZE - 1, 0,
                    ...
                    if (raddr.ss_family == PF_INET) {
                        ssdp_task_config->datagram_buffer[len] = 0;  // Null-terminate whatever we received and treat
                                                                     // like a string...
                        uint16_t remote_port = ((struct sockaddr_in *)&raddr)->sin_port;
                        in_addr_t remote_addr = ((struct sockaddr_in *)&raddr)->sin_addr.s_addr;
                        onPacket(multicast_socket, remote_addr, remote_port, ssdp_task_config->datagram_buffer, len);
                    }
                }
            }
            if (ssdp_task_config) {
                if ((ssdp_task_config->notify_time == 0) ||
                    (ssdp_millis() - ssdp_task_config->notify_time) > (ssdp_task_config->interval * 1000L)) {
                    ssdp_task_config->notify_time = ssdp_millis();
                    ESP_LOGD(TAG, "SSDP: notify...\n");
...
esp_err_t ssdp_start(ssdp_config_t *configuration)
{
    ...
    ssdp_task_config = (ssdp_task_config_t *)calloc(1, sizeof(ssdp_task_config_t));
    if (!ssdp_task_config) {
    ...
    if (err_start == ESP_OK) {
        ...
        ssdp_task_config->datagram_buffer = (char *)calloc(SSDP_DATAGRAM_SIZE, sizeof(uint8_t));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->schema_url) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->schema_url = (char *)calloc(strlen(configuration->schema_url) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->device_type) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->device_type = (char *)calloc(strlen(configuration->device_type) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->friendly_name) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->friendly_name = (char *)calloc(strlen(configuration->friendly_name) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->serial_number) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->serial_number = (char *)calloc(strlen(configuration->serial_number) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->presentation_url) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->presentation_url = (char *)calloc(strlen(configuration->presentation_url) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->manufacturer_name) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->manufacturer_name = (char *)calloc(strlen(configuration->manufacturer_name) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->manufacturer_url) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->manufacturer_url = (char *)calloc(strlen(configuration->manufacturer_url) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->model_name) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->model_name = (char *)calloc(strlen(configuration->model_name) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->model_url) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->model_url = (char *)calloc(strlen(configuration->model_url) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->model_number) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->model_number = (char *)calloc(strlen(configuration->model_number) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->model_description) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->model_description = (char *)calloc(strlen(configuration->model_description) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->server_name) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->server_name = (char *)calloc(strlen(configuration->server_name) + 1, sizeof(char));
        ...
        if (configuration->services_description) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->services_description =
                    (char *)calloc(strlen(configuration->services_description) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ...
        if (configuration->icons_description) {
            ...
            if (err_start == ESP_OK) {
                ssdp_task_config->icons_description = (char *)calloc(strlen(configuration->icons_description) + 1, sizeof(char));
    ...
    if (err_start == ESP_OK) {
        ESP_LOGI(TAG, "Task creation core %d, stack:  %d, priotity %d", configuration->core_id, configuration->stack_size,
                 configuration->task_priority);

        // Task creation
        BaseType_t res =
            xTaskCreatePinnedToCore(ssdp_running_task, "ssdp_running_task", configuration->stack_size, NULL,
                                    configuration->task_priority, &ssdp_task_config->xHandle, configuration->core_id);
...
const char *get_ssdp_schema_str()
{
    ...
    size_t template_size = sizeof(SSDP_SCHEMA_TEMPLATE) + 15  // IP
                           + 5                                // port
    ...
    if (ssdp_task_config->schema) {
        if (sprintf(ssdp_task_config->schema, SSDP_SCHEMA_TEMPLATE, ssdp_get_LocalIP(), ssdp_task_config->port,
