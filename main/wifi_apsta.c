...
esp_err_t wifi_init_apsta(wifi_apsta_config_t* apsta_cfg)
{
    ...
    ESP_RETURN_ON_FALSE(esp_wifi_set_mode(apsta_cfg->wifi_mode) == ESP_OK, ESP_FAIL, TAG, "esp_wifi_set_mode failed");
    ...
    if ((apsta_cfg->wifi_mode == WIFI_MODE_AP) || (apsta_cfg->wifi_mode == WIFI_MODE_APSTA)) {
        ...
        err = esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &ap_event_handler, NULL, NULL);
        ESP_RETURN_ON_FALSE(err == ESP_OK, ESP_FAIL, TAG, "esp_event_handler_instance_register failed");
    }
    ...
    if ((apsta_cfg->wifi_mode == WIFI_MODE_STA) || (apsta_cfg->wifi_mode == WIFI_MODE_APSTA)) {
        ...
        if (apsta_cfg->ap_event_handler != NULL) {
            err = esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, apsta_cfg->ap_event_handler, NULL, NULL);
            ESP_RETURN_ON_FALSE(err == ESP_OK, ESP_FAIL, TAG, "ap event handler register failed");
        }
        if (apsta_cfg->sta_event_handler != NULL) {
            err = esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, apsta_cfg->sta_event_handler, NULL,
                                                      &instance_got_ip);
