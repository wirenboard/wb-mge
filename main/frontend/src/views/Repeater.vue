...
const avgBytesPerSec = computed<number>(() => {
  const s = uptimeS.value;
  if (s <= 0) return 0;
  return Math.round(((forwardBytes.value + reverseBytes.value) / s) * 10) / 10;
});
...
// Format a duration in seconds as HH:MM:SS (hours not padded if > 99 stays as-is).
...
function lineParams(settings: RsSettings | undefined): string {
  if (!settings) return '—';
