...
const inputRows: RegRow[] = [
  ...
  { dec: '337–338', hex: '0x0151–0x0152', regs: 2, type: 'u32', desc: { en: 'Packets processed (since last cache reset)', ru: 'Количество обработанных пакетов (с последнего сброса кэша)' } },
  { dec: '339–340', hex: '0x0153–0x0154', regs: 2, type: 'u32', desc: { en: 'Seconds since the last packet on the bus', ru: 'Секунд с момента последнего пакета на шине' } },
  { dec: '341', hex: '0x0155', regs: 1, type: 'u16', desc: { en: 'Devices currently on the bus (unique slave_ids in cache)', ru: 'Количество устройств на шине (уникальных slave_id в кэше)' } },
  { dec: '342', hex: '0x0156', regs: 1, type: 'u16', desc: { en: 'Average bus poll rate, polls/min', ru: 'Средняя частота опроса шины, опросов/мин' } },
  { dec: '343', hex: '0x0157', regs: 1, type: 'u16', desc: { en: 'Cache value timeout, seconds', ru: 'Таймаут значения кэша, секунды' } },
...
const notes: BiText[] = [
  ...
  { en: 'Bus statistics (337–343) come from the multimaster cache; with the cache inactive these fields read as 0.', ru: 'Статистика шины (337–343) берётся из мультимастер-кэша; при неактивном кэше соответствующие поля читаются как 0.' },
